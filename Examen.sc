...
def estudianteMasConsistente(estudiantes: List[Estudiante]): Unit = {
  val estudiantesValidos: List[(String, Double)] =
    estudiantes
      .filter(e => e.notas.size >= 3)
      .map(e => (e.nombre, e.notas.max - e.notas.min))
  val mejor: (String, Double) = estudiantesValidos.minBy(_._2)
  println("Estudiante más consistente: " + mejor._1)
  println("Consistencia: " + mejor._2)
}